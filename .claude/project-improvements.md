...
**背景**:
- 動画からの音声抽出だけでなく、音声ファイル同士の変換ニーズへの対応

**実装内容**:
- 音声フォーマット（MP3, WAV, FLAC, AAC, OGG, M4A, WMA）の入力サポート
- FFmpeg による音声変換処理
- UI/UXの拡張（ファイル入力・ドラッグ&ドロップ）
...
**背景**:
- 絵文字（emoji）は環境によって表示が異なり、一貫したUXを提供できない
- プロフェッショナルな外観への改善

**実装内容**:
- `HeroIcon.razor` コンポーネントを新規作成
- 全UIコンポーネントのアイコンをSVGインラインに統一
- テーマカラーとの連動対応
...
**学んだこと**:
- SVGインラインはCSSカスタムプロパティ（変数）で色を制御可能
- 外部SVGファイルよりもインラインの方がテーマ切替に対応しやすい

---

### 2025-11-20: 複数動画フォーマット対応

**背景**:
- MOV/MP4以外の動画フォーマットへの対応要望

**実装内容**:
- AVI, MKV, WMV, FLV, WebM フォーマットの入力サポート
- ファイル検証ロジックの強化
...
5. **Web Worker による処理分離**
   - メインスレッドのブロッキング軽減

### ✅ 実装完了

- ~~WASM ライブラリの動作検証~~ → 2025-11-10 完了
- ~~エラーハンドリングの強化~~ → 2025-11-20 ファイルチェック機能として実装
- ~~ダークモード対応~~ → テーマ切り替え機能として実装済み（ライト/ダーク/自動）
